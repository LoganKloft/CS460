...
int fbuf_init()
{
  ...
  *(volatile u32 *)(0x10120010) = (4*1024*1024); // panelBaseAddress
  *(volatile u32 *)(0x10120018) = 0x80B; //82B;  // control register
  *(volatile u32 *)(0x1012001c) = 0x0;           // IntMaskRegister

  fb = (int *)(4*1024*1024);  // at 1MB area; enough for 800x600 SVGA
...
int displayClock(char* clock)
{
  // clear timer
  for (int i = 0; i < 8; i++) unkpchar(cursor, 0, 72 + i);

  // draw timer
  // hh:mm:ss => 7273747576777879
  for (int i = 0; i < 8; i++) kpchar(clock[i], 0, 72 + i);
}